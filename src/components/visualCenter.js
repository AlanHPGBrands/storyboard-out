...
import {base64ImageToRGBMatrix} from './base64ImageToRGBMatrix';
...
const SIZE = 420;
// export 
function doNewVC(rgbMat){
    console.log(rgbMat)
    const height = rgbMat.length;
    const width = rgbMat[0].length;
    const bgColor = normalizeColor(rgbMat[0][0]);
    const {visualLeft,visualTop}=calculateVisualCenter(rgbMat);
    console.log(visualLeft)
