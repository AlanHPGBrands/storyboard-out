...
export const getDesignPatterns = async ( ) => {
    ...
    let returningAllProducts = warsFiles.map( ( warsFile , wIdx ) => {
        ...
        if ( typeof warsFile !== 'undefined' ) {
            tempObject={name:warsFile.key.replace(productNumberImageFolder,'').substring(0,warsFile.key.lastIndexOf('.'))};
        ...
        if(typeof folderVarsFile!=='undefined'){
            ...
            let [productImages,whiteImage]=[prodRegs,/c=BG/i]
                .map(reg=>(
                    ...
                    .filter(f=>reg.test(f.key)&&!/VARS/.test(f.key))
                    .filter(f=>{
...
export const getSvgTextFromIdVersion = ( {
    ...
    return new Promise( ( res , rej ) => {
        ...
        console.log( id , squarePrefix , ver )
        if ( 
            id 
            && typeof id !== 'undefined' 
            && squarePrefix 
